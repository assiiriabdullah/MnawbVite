...
import { gregorianToHijriString, toHijriArabic, addDaysToDate, getHijriInfo } from '../../utils/hijri';
...
export default function Profile() {
  ...
  // Leave request state - now with start_date and num_days instead of end_date
  const [leaveModalOpen, setLeaveModalOpen] = useState(false);
  const [leaveForm, setLeaveForm] = useState({ start_date: '', num_days: 1 });
  ...
  const calculatedEndDate = useMemo(() => {
    if (!leaveForm.start_date || leaveForm.num_days < 1) return '';
    return addDaysToDate(leaveForm.start_date, leaveForm.num_days);
  }, [leaveForm.start_date, leaveForm.num_days]);
  ...
  // Trigger conflict check when form changes
  useEffect(() => {
    if (leaveForm.start_date && calculatedEndDate) {
      checkConflicts(leaveForm.start_date, calculatedEndDate);
    } else {
      setConflictData(null);
    }
  }, [leaveForm.start_date, calculatedEndDate]);

  const handleSubmitLeave = async () => {
    if (!leaveForm.start_date || leaveForm.num_days < 1) {
      toast('يرجى تحديد تاريخ البداية وعدد الأيام', 'error');
      return;
    }
    if (!calculatedEndDate) {
      toast('خطأ في حساب تاريخ النهاية', 'error');
      return;
    }
    setSubmitting(true);
    try {
      ...
      setLeaveForm({ start_date: '', num_days: 1 });
      setConflictData(null);
      // Refresh data
  ...
  // Get Hijri info for the calculated dates
  const startHijriInfo = getHijriInfo(leaveForm.start_date);
  const endHijriInfo = getHijriInfo(calculatedEndDate);
    ...
    <div className="space-y-6">
        ...
        <>
          {canRequestLeave && (
            <div className="flex justify-end">
              <Button onClick={() => { setLeaveModalOpen(true); setLeaveForm({ start_date: '', num_days: 1 }); setConflictData(null); }}>
                  ...
                  <div key={l.id} className="flex items-center justify-between px-5 py-3.5">
                    <div>
                      <p className="text-sm text-gray-800">
                        {toHijriArabic(l.start_date)} → {toHijriArabic(l.end_date)}
                      </p>
                      <p className="text-xs text-gray-400 mt-0.5">
                        {calcDays(l.start_date, l.end_date)} يوم
      ...
      <Modal open={leaveModalOpen} onClose={() => setLeaveModalOpen(false)} title="طلب إجازة جديدة">
        <div className="space-y-4">
          ...
          {/* Number of Days */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">عدد أيام الإجازة</label>
            <div className="flex items-center gap-3">
              <button
                type="button"
                onClick={() => setLeaveForm(prev => ({ ...prev, num_days: Math.max(1, prev.num_days - 1) }))}
                ...
                <input
                  ...
                  onChange={e => {
                    ...
                    if (!isNaN(val) && val >= 1 && val <= 365) {
                      setLeaveForm(prev => ({ ...prev, num_days: val }));
                    }
                  }}
                  className="w-full text-center px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition outline-none text-gray-800 text-lg font-bold"
                />
              </div>
              <button
                type="button"
                onClick={() => setLeaveForm(prev => ({ ...prev, num_days: Math.min(365, prev.num_days + 1) }))}
                className="w-10 h-10 rounded-xl bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-600 transition text-lg font-bold"
              >
                +
              </button>
            </div>
            <p className="text-xs text-gray-400 mt-1 text-center">يوم</p>
          </div>

          {/* Calculated End Date */}
          {leaveForm.start_date && calculatedEndDate && (
            <div className="bg-gradient-to-l from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl p-4 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-600">تاريخ نهاية الإجازة:</span>
              </div>
              <div className="text-center">
                {endHijriInfo && (
                  <p className="text-lg font-bold text-emerald-700">
                    {endHijriInfo.hijriArabic} هـ
                  </p>
                )}
                <p className="text-xs text-gray-400 mt-1">
                  {calculatedEndDate} ميلادي
                </p>
              </div>
              <div className="flex items-center justify-between pt-2 border-t border-emerald-200/50">
                <span className="text-xs text-gray-500">المدة الإجمالية:</span>
                <span className="text-sm font-bold text-emerald-600">{leaveForm.num_days} يوم</span>
              </div>
            </div>
          )}
                      ...
                      {conflictData.conflicts.map((c, i) => (
                        <div key={i} className="px-3 py-2 rounded-xl bg-amber-50 border border-amber-200 text-xs">
                          ...
                          <p className="text-amber-600 mt-0.5">
                            إجازته: {toHijriArabic(c.leave_start)} → {toHijriArabic(c.leave_end)}
