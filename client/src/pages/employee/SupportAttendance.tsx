...
export default function EmpSupportAttendance() {
  ...
  useEffect(() => {
    ...
    api('/api/attendance/signature').then(d => setSavedSig(d.signature_data)).catch(() => {});
  }, []);

  const loadReport = async () => {
    setLoading(true);
    try {
      const data = await api('/api/support-attendance/reports/today');
  ...
  const toggleCheckin = async (sessionId: number, currentStatus: string) => {
    ...
    const endpoint = action === 'open' ? 'open-checkin' : 'close-checkin';
    try {
      await api(`/api/support-attendance/sessions/${sessionId}/${endpoint}`, 'PUT', { signature: 'sig' });
  ...
  const toggleCheckout = async (sessionId: number, currentStatus: string) => {
    const action = currentStatus === 'pending' ? 'open' : 'completed';
    const endpoint = action === 'open' ? 'open-checkout' : 'close-checkout';
    try {
      await api(`/api/support-attendance/sessions/${sessionId}/${endpoint}`, 'PUT', { signature: 'sig' });
  ...
  const loadGroupRecords = async (sessionId: number, group: string) => {
    ...
    if (session && session.records) {
        setGroupRecords(session.records);
    } else {
        setGroupRecords([]);
    }
  };
  ...
  const updateRecordStatus = async (recordId: number, newStatus: string) => {
    try {
      ...
      loadReport(); // Reload all data to ensure freshness
      // Also update local groupRecords for immediate feedback
      setGroupRecords(prev => prev.map(r => r.id === recordId ? { ...r, status: newStatus } : r));
    } catch (e: any) { toast(e.message, 'error'); }
  };
  ...
  const submitReport = async () => {
    ...
    if (!sig) { toast('توقيع المشرف مطلوب لرفع الكشف', 'error'); return; }
    try { await api(`/api/support-attendance/reports/${report.id}/submit`, 'PUT', { signature: sig }); toast('تم رفع الكشف للإدارة ✓'); loadReport(); }
    catch (e: any) { toast(e.message, 'error'); }
  };
          ...
          <>
            ...
            {report.status === 'active' && (
              <div className="space-y-4">
                <Card>
                  <h3 className="font-bold text-gray-800 mb-4">توقيع المشرف لاعتماد الكشف</h3>
              ...
              <div className="space-y-4">
                ...
                {!hasCheckedOut && (
                  <>
                    <SignaturePad ref={sigRef} initialData={savedSig} />
                    <div className="flex gap-3">
                      {!hasCheckedIn && myStatus?.session?.checkin_status === 'open' && (
                        <Button onClick={supportCheckIn} loading={submitting} className="flex-1">
                          <LogIn size={16} /> تسجيل الحضور
                        </Button>
                      )}
                      {hasCheckedIn && !hasCheckedOut && myStatus?.session?.checkout_status === 'open' && (
