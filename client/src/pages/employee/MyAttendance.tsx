...
export default function MyAttendance() {
  ...
  const superSigRef = useRef<SignaturePadRef>(null);
  ...
  const approveSession = async () => {
    if (!session) return;
    const sig = superSigRef.current?.getData() || sigRef.current?.getData();
                ...
                {session.status !== 'approved' && (
                  <div className="space-y-4 mt-6 border-t border-gray-100 pt-4">
                    <Card>
                      ...
                      <SignaturePad ref={superSigRef} initialData={savedSig} />
                    </Card>
                    <div className="flex gap-3">
