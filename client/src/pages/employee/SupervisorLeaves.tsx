...
export default function SupervisorLeaves() {
  ...
  // Filter leaves by selected Hijri month
  const monthFilteredLeaves = useMemo(() => {
    return leaves.filter(l => leaveOverlapsHijriMonth(l.start_date, l.end_date, selectedMonth));
                ...
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-gray-600 mt-0.5">
                      {toHijriArabic(l.start_date)} → {toHijriArabic(l.end_date)} • {calcDays(l.start_date, l.end_date)} يوم
                    </p>
                    <p className="text-xs text-gray-400 mt-0.5">
      ...
      <Modal open={settingsModalOpen} onClose={() => setSettingsModalOpen(false)} title="إعدادات الحد الشهري">
        <div className="space-y-4">
          <p className="text-sm text-gray-500">
            حدد الحد الأقصى لعدد الموظفين في إجازة لكل شهر هجري في مناوبتك.
          </p>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">الشهر الهجري</label>
            <div className="px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-800">
              {hijriMonthName(settingMonth)}
            </div>
            <p className="text-xs text-gray-400 mt-1">يمكنك تغيير الشهر من خلال التنقل في الصفحة الرئيسية</p>
          </div>
          <div>
              ...
              <div className="space-y-1.5 max-h-32 overflow-y-auto">
                {settings.map((s, i) => (
                  <div key={i} className="flex items-center justify-between px-3 py-1.5 rounded-lg bg-gray-50 text-xs">
                    <span className="text-gray-600">{hijriMonthName(s.month)}</span>
